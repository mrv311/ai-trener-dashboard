...
const s = [];
s.push({duration: 15*60, power: 55});
for(let i=0; i<9; i++) {
  s.push({duration: 3*60, power: 120});
  s.push({duration: 5*60, power: 50});
}
s.push({duration: 3*60, power: 50});
let cat = categorizeWorkout(s);
console.log(calculateCategoryDifficulty(s, cat));
