...
export default function AnalyticsTab({ intervalsId, intervalsKey }) {
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      ...
      try {
        ...
        const response = await fetch(`https://intervals.icu/api/v1/athlete/${intervalsId.trim()}/activities?oldest=${oldest}&newest=${newest}`, {
        ...
        const validActivities = data.filter(act => (act.type === 'Ride' || act.type === 'VirtualRide') && act.icu_training_load > 0);
  ...
  const zoneData = useMemo(() => {
    ...
    activities.forEach(act => {
      const targetZones = zoneType === 'power' ? act.icu_zone_times : act.icu_hr_zone_times;
      if (targetZones && Array.isArray(targetZones) && targetZones.length > 0) {
        targetZones.forEach((secs, i) => {
          if (i < 7) {
            zones[i] += (secs || 0);
  ...
  // POPRAVAK: Sada koristimo icu_ftp umjesto icu_pm_ftp
  ...
  const renderCustomizedLabel = ({ cx, cy, midAngle, outerRadius, percent }) => {
    ...
    return (
      <text x={x} y={y} fill="#57534e" textAnchor={x > cx ? 'start' : 'end'} dominantBaseline="central" fontSize="11px" fontWeight="bold">
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-3 border border-stone-200 shadow-sm rounded-lg">
          ...
          <p className="font-black text-orange-600 text-xl">{payload[0].value} <span className="text-sm font-bold text-stone-400">W</span></p>
        </div>
      );
    }
    return null;
    ...
    <div className="max-w-[1600px] mx-auto flex flex-col gap-6 animate-in fade-in h-full pb-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between shrink-0 gap-4">
        <div className="flex items-center gap-4">
          <div className="bg-orange-100 p-3 rounded-xl"><BarChart2 className="w-6 h-6 text-orange-600"/></div>
          <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-6 flex flex-col relative min-h-[400px]">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-stone-800 font-bold flex items-center gap-2"><PieChartIcon className="w-4 h-4 text-orange-500" /> Distribucija</h3>
              
              <div className="flex items-center bg-stone-100 rounded-lg p-1 border border-stone-200">
                <button 
                  onClick={() => setZoneType('power')}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-colors ${zoneType === 'power' ? 'bg-white text-orange-600 shadow-sm' : 'text-stone-400 hover:text-stone-600'}`}
                >
                  <Zap className="w-3 h-3" /> Snaga
                </button>
                <button 
                  onClick={() => setZoneType('hr')}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-wider transition-colors ${zoneType === 'hr' ? 'bg-white text-rose-500 shadow-sm' : 'text-stone-400 hover:text-stone-600'}`}
                >
              ...
              <div className="flex-1 w-full relative">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    ...
                    >
                      {zoneData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                    </Pie>
                    <RechartsTooltip />
                    <Legend verticalAlign="bottom" height={80} iconType="circle" wrapperStyle={{ fontSize: '11px', fontWeight: 'bold' }} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
            ) : (
              <div className="flex-1 flex items-center justify-center text-stone-400 text-sm italic text-center px-4 bg-stone-50 rounded-xl border border-stone-100 mt-4">
                Nema zabilježenih podataka o zonama {zoneType === 'power' ? 'snage' : 'pulsa'} za odabrani period.
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-6 flex flex-col lg:col-span-2 relative min-h-[400px]">
             <h3 className="text-stone-800 font-bold mb-1 flex items-center gap-2"><Activity className="w-4 h-4 text-orange-500" /> Konzistentnost Treninga</h3>
             <p className="text-[11px] text-stone-400 font-medium mb-6 uppercase tracking-widest">TSS i Sati po tjednima</p>
             
             <div className="flex-1 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={weeklyData} margin={{ top: 10, right: 30, left: -20, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e7e5e4" />
                    <XAxis dataKey="name" tick={{ fontSize: 11, fill: '#78716c', fontWeight: 600 }} tickMargin={10} axisLine={false} tickLine={false} />
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-stone-200 p-6 flex flex-col lg:col-span-3 relative h-[350px]">
             <h3 className="text-stone-800 font-bold mb-1 flex items-center gap-2"><TrendingUp className="w-4 h-4 text-orange-500" /> Povijest FTP-a</h3>
             <p className="text-[11px] text-stone-400 font-medium mb-6 uppercase tracking-widest">Kretanje tvoje kondicije kroz odabrani period</p>
               ...
               <div className="flex-1 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={ftpData} margin={{ top: 10, right: 20, left: -20, bottom: 0 }}>
                      <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e7e5e4" />
                      <XAxis dataKey="displayDate" tick={{ fontSize: 11, fill: '#78716c', fontWeight: 600 }} tickMargin={10} axisLine={false} tickLine={false} minTickGap={30} />
                      <YAxis domain={['dataMin - 10', 'dataMax + 10']} tick={{ fontSize: 11, fill: '#78716c' }} axisLine={false} tickLine={false} />
                      <RechartsTooltip content={<CustomFtpTooltip />} cursor={{ stroke: '#facc15', strokeWidth: 2, strokeDasharray: '4 4' }} />
                      <Line type="stepAfter" dataKey="ftp" name="FTP (W)" stroke="#ea580c" strokeWidth={3} dot={{ r: 4, fill: '#fff', stroke: '#ea580c', strokeWidth: 2 }} activeDot={{ r: 7, fill: '#ea580c', strokeWidth: 0 }} />
                    </LineChart>
                  </ResponsiveContainer>
               </div>
             ) : (
