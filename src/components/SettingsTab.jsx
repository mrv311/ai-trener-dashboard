...
export default function SettingsTab({ intervalsId, setId, intervalsKey, setKey, profile, setProfile, onSave }) {
  ...
  return (
    <div className="max-w-5xl mx-auto bg-white rounded-xl shadow-sm border border-stone-200 animate-in fade-in flex flex-col min-h-[700px]">
      <div className="flex border-b border-stone-200 px-8 pt-6 gap-6 bg-stone-50/50 rounded-t-xl">
        <button onClick={() => setSubTab('personal')} className={`pb-4 text-sm font-bold uppercase tracking-widest border-b-2 transition-colors ${subTab === 'personal' ? 'border-orange-600 text-orange-700' : 'border-transparent text-stone-500 hover:text-stone-800'}`}>Osobne Postavke</button>
        <button onClick={() => setSubTab('connections')} className={`pb-4 text-sm font-bold uppercase tracking-widest border-b-2 transition-colors ${subTab === 'connections' ? 'border-orange-600 text-orange-700' : 'border-transparent text-stone-500 hover:text-stone-800'}`}>Veze (Connections)</button>
      </div>
      ...
      <div className="p-8">
        ...
        {subTab === 'connections' && (
          <div className="space-y-6 max-w-xl animate-in fade-in">
            <p className="text-stone-500 mb-8 text-sm">Povežite se s Intervals.icu API-jem.</p>
            <div><label className="block text-[10px] font-black text-stone-500 uppercase tracking-widest mb-2">Intervals ID</label><input type="text" value={intervalsId} onChange={e => setId(e.target.value)} className="w-full bg-stone-50 border border-stone-200 rounded-lg p-3 outline-none focus:border-orange-500 font-mono" placeholder="npr. 15670332" /></div>
            <div><label className="block text-[10px] font-black text-stone-500 uppercase tracking-widest mb-2">API Key</label><input type="password" value={intervalsKey} onChange={e => setKey(e.target.value)} className="w-full bg-stone-50 border border-stone-200 rounded-lg p-3 outline-none focus:border-orange-500 font-mono" placeholder="Zalijepi API ključ" /></div>
            <div className="pt-4"><button onClick={saveSettings} className="bg-orange-600 hover:bg-orange-700 text-white px-8 py-3 rounded-lg font-bold shadow-sm transition-colors text-sm uppercase tracking-wider">Spremi i Poveži</button></div>
          </div>
        )}
      </div>
    </div>
