...
export default function TrainerGraph({
  ...
  return (
    <div className="flex flex-col flex-1 w-full gap-2">
      {/* Glavni Graf */}
      <div className="relative flex-1 w-full bg-stone-50 rounded-t-xl border border-stone-200">
        {/* Sloj za overlay i SVG — clipan */}
        <div className="absolute inset-0 overflow-hidden rounded-t-xl pointer-events-none">
          <div className="absolute top-0 bottom-0 left-0 bg-stone-900/30 z-10 transition-all duration-1000 ease-linear" style={{ width: `${progressPercent}%` }} />
        ...
        <div className="relative flex items-end w-full h-full z-30">
          {workoutRecipe.map((step, i) => {
            ...
            return (
              <div
                ...
                className={`${getZoneColorForTrainer(step.power)} border-r border-white/20 transition-all duration-300 opacity-90 relative cursor-pointer hover:opacity-100`}
              >
                {/* Tooltip */}
                {hoveredStep && hoveredStep.index === i && (
                  <div
                    className="absolute bottom-full mb-2 bg-stone-900 text-white text-[11px] font-bold px-3 py-2 rounded-lg shadow-xl pointer-events-none whitespace-nowrap z-[100]"
                    style={{
                  ...
                  >
                    ...
                    <span className="block mt-1 font-normal text-stone-400">{Math.floor(step.duration / 60)} min {step.duration % 60} sec</span>
                  </div>
                )}
              </div>
            );
